...
const StepOneBasicInfo = ({ isVerified, setFinalAnalysis }: StepOneBasicInfoProps) => {
  ...
  const handleAnalyze = () => {
    ...
    if (!currentDescription.trim()) {
      alert("Please enter a description before analyzing.");
      return;
    }
    analyze(
      { description: currentDescription },
      {
        onSuccess: (data) => {
          ...
          if (isVerified) {
            ...
            alert("Analysis complete! The result will be attached to your campaign.");
          }
        },
        onError: (error) => alert(`Analysis failed: ${error.message}`),
      },
    );
  };
            ...
            <div className="mt-3">
              ...
              {!isVerified && (
                <p className="text-xs text-muted-foreground mt-2">
                  Guardian analysis will run automatically upon submission for unverified users.
                </p>
              )}
            </div>
          </div>
      ...
      {analysisResult && (
        <GuardianAnalysisModal
          ...
          onConfirm={() => {
            ...
            alert("Analysis has been attached to your campaign.");
          }}
          onRetry={() => {
